...
function connectCheckoutForm() {
  ...
  if (payNowBtn) {
    payNowBtn.addEventListener("click", async () => {
      ...
      const payment = data.get("payment") || "M-Pesa";
      const deliveryMode = data.get("deliveryMode") || "ship";
      const location = String(data.get("location") || "").trim();
      const pickupLocation = String(data.get("pickupStore") || "").trim();
      ...
      if (payment === "DPO") {
        ...
        const requiredDestination = deliveryMode === "pickup" ? pickupLocation : location;

        if (!name || !phone || !email || !requiredDestination) {
        ...
        const amount = Number(totals[currency] || 0) + shipping.shippingFee;
        ...
        const payload = {
          ...
          shipping: {
            mode: deliveryMode,
            location: deliveryMode === "pickup" ? pickupLocation : location,
            method: shipping.shippingLabel,
            fee: shipping.shippingFee
          },
          items: cart.map((item) => ({
            id: item.id,
            name: item.name,
        ...
        try {
          ...
          payNowBtn.textContent = payment === "DPO" ? "Pay with Card" : payment === "PayPal" ? "Pay with PayPal" : "Pay with M-Pesa";
        }
        return;
      }
      ...
      alert("Please choose M-Pesa, Card, or PayPal to continue.");
    });
  }
